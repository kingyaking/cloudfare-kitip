...
#请设置期望的带宽大小(默认10,单位 Mbps)
bandwidth=10
#请设置RTT测试进程数(默认10,最大50)
tasknum=10
...
#优选完毕后替换passwall节点ID再启动passwall服务
#passwall节点ID查看,运行命令 cat /etc/config/passwall 查看config nodes单引号里面的字符串，替换下面的xxxxxxxxxx
uci set passwall.c885dd5f9aa14c448557f79f4f43fdd4.address=$anycast
uci commit passwall
/etc/init.d/haproxy start
/etc/init.d/passwall start
echo "passwall的ip已经更新"
if [ $pushplus == 1 ]
then
	curl --location --request POST 'https://www.pushplus.plus/send' --header 'Content-Type: application/json' --data-raw '{"token":"'"$token"'","title":"'"$isp"'CF优选IP通知","content":"优选IP '"$anycast"'\n公网IP '"$publicip"'\n设置带宽 '"$bandwidth"' Mbps\n实测带宽 '"$realbandwidth"' Mbps\n峰值速度 '"$max"' kB/s\n往返延迟 '"$avgms"' 毫秒\n数据中心 '"$colo"'\n总计用时 '"$[$endtime-$starttime]"' 秒","template":"txt"}'
fi
}
...
#先停止passwall服务
	...
	do
		if [ "$ips" == "ipv4" ]
		then
			http_code=$(curl -A "" -s http://$anycast:443 -o /dev/null --connect-timeout 3 --max-time 5 -w %{http_code})
		else
			http_code=$(curl -A "" -s http://[$anycast]:443 -o /dev/null --connect-timeout 3 --max-time 5 -w %{http_code})
		fi
		if [ "$http_code" != "400" ]
		then
			echo "$(date +'%H:%M:%S') $anycast 发生故障"
			endtime=$(date +%s)
			if [ $pushplus == 1 ]
			then
				curl --location --request POST 'https://www.pushplus.plus/send' --header 'Content-Type: application/json' --data-raw '{"token":"'"$token"'","title":"'"$anycast"' 无法访问","content":"故障时间：'"$(date +'%Y-%m-%d %H:%M:%S')"'\n故障IP：'"$anycast"'\n故障原因：无法连接服务器\n监测节点：'"$isp"'\n存活时间：'"$[$endtime-$starttime]"' 秒","template":"txt"}'
			fi
			break
		else
			echo "$(date +'%H:%M:%S') $anycast 状态正常"
			sleep 5
		fi
	done
done
